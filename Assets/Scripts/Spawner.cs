...
public class Spawner : MonoBehaviour
{
    ...
    /// <summary>
    /// Obstacles will have higher probability to spawn at the bottom
    /// </summary>
    public bool spawnBottomBiased = true;
    ...
    public void SpawnObstacle()
    {
        ...
        float3 size = (1.0f - spawnSize) / 2.0f;
        switch (RandomIntInRange(lastSpawnDown ? 1 : 0, 2, !lastSpawnDown && spawnBottomBiased))
        {
            case 0:
                obstacle.transform.position += (Vector3)(-spawnOffset - size);
                lastSpawnDown = true;
                break;
            case 1:
                obstacle.transform.position += (Vector3)(-size);
                lastSpawnDown = false;
                break;
            default:
                obstacle.transform.position += (Vector3)(spawnOffset + size);
    ...
    /// <returns>Return a random int in a range</returns>
    public static int RandomIntInRange(int min, int max, bool minBiased = false){
        int value = Random.Range(minBiased ? min - 1 : min, max + 1);
